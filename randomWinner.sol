...
contract rngETH {
    ...
    uint256[9999999] public winnerResult;
    string[9999999] public competitionName;
    string[9999999] public compContestants;
    
    function getResults (uint256 competitionID) public view returns (uint256 winnerByID, string memory competitionNameByID, string memory compContestantsByID){
        winnerByID = winnerResult[competitionID];
        competitionNameByID = competitionName[competitionID];
        compContestantsByID = compContestants[competitionID];
    }
    
    function getRandomWinner(string memory referenceName, int256 contestantCount, string memory contestantNames) payable public {
        require(msg.value == 2 finney);    
        totalCount = totalCount + 1;

        bytes32[] memory pool = Random.initLatest(1, 0);        
        
	    uint256 winnerWinnerChickenDinner = uint256(Random.uniform(pool, 1, contestantCount)); 
	    
        winnerResult[totalCount] = winnerWinnerChickenDinner;
        competitionName[totalCount] = referenceName;
        compContestants[totalCount] = contestantNames;
